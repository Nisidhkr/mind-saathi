...
const PeriodTracker = () => {
  ...
  const fetchPeriods = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/periods');
  ...
  const handleFormSubmit = async (periodData) => {
    try {
      if (editingPeriod) {
        await axios.put(`http://localhost:5000/api/periods/${editingPeriod._id}`, periodData);
      } else {
        await axios.post('http://localhost:5000/api/periods', periodData);
      }
  ...
  const handleDeletePeriod = async (periodId) => {
    if (window.confirm('Are you sure you want to delete this period entry?')) {
      try {
        await axios.delete(`http://localhost:5000/api/periods/${periodId}`);
        fetchPeriods();
      } catch (error) {
